...
bool UNav3DComponent::FindVolume()
{
	...
	for (const auto& NavVolume : Volumes)
	{
		if (ANav3DVolume* CurrentVolume = Cast<ANav3DVolume>(NavVolume))
		{
...
void UNav3DComponent::TickComponent(const float DeltaTime, const ELevelTick TickType,
	...
	if (bCheckLineOfSight)
	{
		if (FVector::Dist(StartLocation, TargetActor->GetActorLocation()) <= MinimumDistance)
		{
			...
			if (HitResult.bBlockingHit && HitResult.GetActor() == TargetActor)
			{
				...
				if (bDebugLogPathfinding) UE_LOG(LogTemp, Error,
				                                 TEXT(
					                                 "%s: Find line of sight unnecessary. Target is already visible to Nav3D component owner"
				                                 ), *GetOwner()->GetName());
#endif
...
#if WITH_EDITOR
	if (bDebugLogPathfinding) UE_LOG(LogTemp, Display, TEXT("%s: Find line of sight task called successfully"),
	                                 *GetOwner()->GetName());
#endif
}
...
#if WITH_EDITOR
		if (bDebugFindCover) UE_LOG(LogTemp, Error,
		                            TEXT(
			                            "%s: Find cover cannot initialise. Nav3D component owner is not inside a Nav3D volume"
		                            ), *GetOwner()->GetName());
#endif
...
#if WITH_EDITOR
		if (bDebugLogPathfinding) UE_LOG(LogTemp, Error,
		                                 TEXT("%s: Find cover cannot initialise. Nav3D octree has not been built"),
		                                 *GetOwner()->GetName());
#endif
...
#if WITH_EDITOR
		if (bDebugFindCover) UE_LOG(LogTemp, Error,
		                            TEXT("%s: Find cover cannot initialise. Nav3D volume cover map is not enabled"),
		                            *GetOwner()->GetName());
#endif
...
#if WITH_EDITOR
		if (bDebugFindCover) UE_LOG(LogTemp, Error,
		                            TEXT("%s: Find cover cannot initialise. Nav3D volume cover map is invalid or empty"
		                            ), *GetOwner()->GetName());
#endif
	...
	FNav3DCoverLocation& CoverLocation) const
{
	...
	for (auto& Component : OverlappedComponents)
	{
		...
		{
			UActorComponent* ActorComponent = Component->GetOwner()->GetComponentByClass(
				UNav3DOcclusionComponent::StaticClass());
		...
		for (auto& FoundLocation : FoundLocations)
		{
			...
			if (!Volume->IsCoverLocationValid(FoundLocation)) continue;
			CoverLocations.Add(FNav3DCoverLocation(Component->GetOwner(), FoundLocation, Normal));
			if (bPerformLineTraces)
			{
				IndexedDistances.Add(
					TPair<int32, float>(D, FVector::DistSquared(FoundLocation, GetOwner()->GetActorLocation())));
				D++;
			}
			else
			{
	...
	if (!bPerformLineTraces)
	{
		...
		switch (SearchType)
		{
		default: UKismetMathLibrary::MinOfFloatArray(Distances, Index, SquareDistance);
			break;
		case ENav3DCoverSearchType::Furthest: UKismetMathLibrary::MaxOfFloatArray(Distances, Index, SquareDistance);
			break;
		case ENav3DCoverSearchType::Random: Index = UKismetMathLibrary::RandomIntegerInRange(0, Distances.Num() - 1);
			break;
		}
		if (Index != -1)
		{
			CoverLocation = CoverLocations[Index];
		}
		return;
	}
	...
	{
	default:
		IndexedDistances.Sort([](const TPair<int32, float>& A, const TPair<int32, float>& B)-> bool
		{
...
#if WITH_EDITOR
	if (bDebugLogPathfinding) UE_LOG(LogTemp, Error, TEXT("%s: Pathfinding failed after %i iterations"),
	                                 *GetOwner()->GetName(), I);
#endif
}
...
void UNav3DComponent::ApplyPathLineOfSight(FNav3DPath& Path, const AActor* Target, const float MinimumDistance) const
{
	...
	{
		if (FVector::Dist(PathPoints[Index], Target->GetActorLocation()) > MinimumDistance)
		{
			continue;
		}
		FCollisionQueryParams CollisionQueryParams;
...
{
	if (!GetWorld() || Config.PathPruning == ENav3DPathPruning::None || Path.Points.Num() < 3 || !GetOwner()->
		IsValidLowLevel())
	{
		return;
	}
...
{
	if (Config.PathSmoothing < 1 || Path.Points.Num() < 3)
	{
		return;
	}
	TArray<FVector> PathPoints;
	...
	for (int32 Index = 0; Index < PathPoints.Num(); Index++)
	{
		...
		for (int I = 1; I <= Config.PathSmoothing; I++)
		{
			...
			SplinePoints.Add(FNav3DPathPoint(0.5f * (A + (B * T) + (C * T * T) + (D * T * T * T)),
			                                 Path.GetPoints()[Index - 1].Layer));
		}
	}
...
{
	if (!GetWorld() || !Volume || Path.Points.Num() < 2 || !bDebugDrawEnabled)
	{
		return;
	}
	...
	{
		DebugPath.Points.Add(Point.Location);
	}
...
{
	if (!GetWorld() || !Volume || !bDebugDrawEnabled || CoverLocation.Location == FVector::ZeroVector)
	{
		return;
	}
